...
def main():
    ...
    try:
        rospy.init_node('go_to_random_position')
        limb = Limb()
        ...
        if (args.position is None and args.orientation is None
            ...
            z_diff = Max_Z - Min_Z
            pose.position.x = random.uniform(Min_Y, Max_Y)
            pose.position.y = Min_X
            pose.position.z = random.uniform(Min_Z + 0.5*(z_diff), Max_Z)
            pose.orientation.x = 0.710
            pose.orientation.y = -0.015
            pose.orientation.z = 0.272
            pose.orientation.w = 0.285
